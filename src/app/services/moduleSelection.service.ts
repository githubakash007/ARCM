...
import { apiModuleSelectionUrl } from "../shared/constant/featureBasedApiUrl.constant";
...
export class ModuleSelectionService {
    ...
    getModules(inputFilters:IKeyValue[],filterKey = '', sortOrder = 'asc', pageNumber = 0, pageSize = 10): void {
        ...
        this._httpClient.post<IModule[]>(this.getRelativeUrl(ModuleApiName.getBaseModules), inputData, httpOptions).pipe(
            catchError(() => of(null)),
    ...
    private getRelativeUrl(relativeUrl: ModuleApiName | string): string {
        let featureUrl = apiModuleSelectionUrl;
